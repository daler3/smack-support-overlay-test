...
static int smack_inode_copy_up(struct dentry *dentry, struct cred **new)
{
	...
	printk(KERN_DEBUG "Debug notice-- Inside smack_inode_copy_up_xattr");

	if (new_creds == NULL) {
		printk(KERN_DEBUG "Debug notice-- Inside smack_inode_copy_up");
		new_creds = prepare_creds(); 
		if (new_creds == NULL){
			printk(KERN_DEBUG "Debug notice --Inside 2nd if: Inside smack_inode_copy_up\n");
			return -ENOMEM;
		}
	}
...
static int smack_inode_copy_up_xattr(const char *name)
{
	...
	*/
	if (strcmp(name, XATTR_NAME_SMACK) == 0){ 
		printk(KERN_DEBUG "Debug Notice --Inside if: Inside smack_inode_copy_up_xattr\n");
		return 1; 
	}
				 	...
				 	struct cred *new)
{
	...
	printk(KERN_DEBUG "Debug Notice-- Inside smack_dentry_create_files_as\n");
